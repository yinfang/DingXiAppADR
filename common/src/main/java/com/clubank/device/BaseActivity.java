...
//        amapListener = new MyAmapListener();
//        if (C.location == null) {
//            location();
//        }
...
//        }
    ...
    protected void onResume() {
        ...
        restoreVars();
        boolean checkVersion = settings.getBoolean("checkVersion", false);
        if (checkVersion) {
            setTimer();
        }
    }

    public void setTimer() {//5分钟检测一次新版本
        if (null != task) {
            task.stop();
        }
        task = new MyTimeTask(1000 * 60 * 5, new TimerTask() {
            @Override
            public void run() {//检测新版本
                new MyAsyncTask(BaseActivity.this, ViewVersion.class, false).run(VersionUtil.getVersionCode(BaseActivity.this));
            }
        });
        task.start();
    }
...
//        String fname = settings.getString("fname", "");
    ...
    public void onPostExecute(Class<?> op, Result result) {
        if (op == ViewVersion.class) {//版本更新
            if (!checkNull(result.obj.toString())) {
                MyRow row = ((MyData) result.obj).get(0);
                checkVersionUpdate(row);
            }
        } else {
            if (result.code == BRT.AUTH_ERROR.getCode()) {
                if (!hasGet304Status) {
                    hasGet304Status = true;
                    UI.showToast(this, "请重新登录");
    ...
    private void checkVersionUpdate(final MyRow row) {
        if (row == null) {
            return;
        }
        int version = Integer.parseInt(row.getString("buildVerion"));
        if (VersionUtil.shouldUpdate(this, version)) {
            final AlertDialog ad = new AlertDialog(this);
            ad.setTitle(String.format(getString(R.string.found_new_version), "\n是否立即更新?"));
            ad.setNegativeButton("下次再说", 0, 0, new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    saveSetting("checkVersion", false);//下次进入app开启版本检测功能
                    ad.dismiss();
                }
            });
            ad.setPositiveButton("立即更新", 0, 0, new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ad.dismiss();
                    new DownloadFile(BaseActivity.this).execute(row.getString("packageUrl"));
//                    new DownloadFile(BaseActivity.this).execute("http://192.168.0.46/app.apk");
//                    saveSetting("checkVersion", false);//已下载不检测
                }
            });
        }
    }
